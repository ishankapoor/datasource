...
export class GenericDatasource {
  constructor(instanceSettings, $q, backendSrv, templateSrv) {
    ...
    this.urlReport = this.urlDatasource + '/swarm/NETIM_NETWORK_METRIC_DATA_SERVICE/api/v1/network-metric-data';
  ...
  query(options) {
    ...
    for (let i in query.targets) {
      ...
      dataDefs = {
        ...
        'metricClass': metricClassSelected,
        'metrics':[],
        'elementIds': [objectSelected],
        'sortOrder': 'ASCENDING',
        'elementType': 'VNES_OE',
        'includeRaw': true,
        'rollupCriterias': ['aggregateMinRollup','aggregateMaxRollup','aggregateAvgRollup','aggregatePctlRate95Rollup','aggregatePctlRate98Rollup'],
        'aggregations': [],
        'includeSamples': true,
        'pageSize':2147483647
      }
    ...
    // Earlier, duplicated and incomplete request were deleted, we need to rebuilt them
      ...
      for (let currentQuery in responseQuery) {
        ...
        if (typeof responseQuery[currentQuery].data.metricData === 'undefined') {
          console.log('###### DATA POINT ERROR FOR ROW ' + currentQuery + '  #####')
          continue;
        }

        if (typeof responseQuery[currentQuery].data.metricData.metricElementDataList === 'undefined') {
          console.log('###### DATA POINT ERROR FOR ROW ' + currentQuery + '  #####')
          continue;
        }

        if (typeof responseQuery[currentQuery].data.metricData.metricElementDataList[0] === 'undefined') {
        ...
        metricSplitted = query.targets[currentQuery].metricID.split(' {elementId}')
        metricHere = metricSplitted[1]

        // valeurs is an array containing all data requested (rollup, timestamp, metric....)
        let valeurs = responseQuery[currentQuery].data.metricData.metricElementDataList[0].timestampToValuesMap

        // Need to delete first empty datapoints
        let untilAllEmptyDatapointsAreDeleted = false 
        ...
        let allKeys = Object.keys(responseQuery[currentQuery].data.elementIdToObjectInfoMap)
        allKeysWithId = []
